...
public class UtilEnum {
	...
	public  enum ESTADO_REGISTRO_BASE{
		...
		public static ESTADO_REGISTRO_BASE getEstadoRegistroByID(Integer id){
			UtilEnum.ESTADO_REGISTRO_BASE[] valores = ESTADO_REGISTRO_BASE.values();
		...
		public static String getEstadoKeyMsg(Integer estado){
			String codeMsg =  null;
			for(UtilEnum.ESTADO_REGISTRO_BASE item : UtilEnum.ESTADO_REGISTRO_BASE.values() ){
				if(item.getCodigo() ==estado){
	...
	public  enum TIPO_BENEFICIO{
		...
		public static String getTipoBeneficioKeyMsg(Integer estado){
			String descripcion =  null;
			for(UtilEnum.TIPO_BENEFICIO item : UtilEnum.TIPO_BENEFICIO.values() ){
				if(item.getCodigo() ==estado){
	...
	public  enum TIPO_SEGMENTO{
		...
		public static String getTipoSegmentoTexto(Integer estado){
			String descripcion =  null;
			for(UtilEnum.TIPO_SEGMENTO item : UtilEnum.TIPO_SEGMENTO.values() ){
				if(item.getCodigo() ==estado){
					descripcion = item.getTexto();
					break;
				}
...
//	}
	...
	public  enum TIPO_VENTA{
		...
		public static TIPO_VENTA getTipoVenta(Integer codigo){
			UtilEnum.TIPO_VENTA[] valores = TIPO_VENTA.values();
			for(int i=0; i<valores.length; i++){
	...
	public  enum TIPO_TRACKING{
		...
		public static TIPO_TRACKING getTipoTracking(int codigo){
			UtilEnum.TIPO_TRACKING[] valores = TIPO_TRACKING.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public  enum ESTADO_VENTA{
		...
		public static ESTADO_VENTA getEstadoVentaByID(Integer id){
			UtilEnum.ESTADO_VENTA[] valores = ESTADO_VENTA.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo() == id){
					return  valores[i];
				}
	...
	public  enum DELIVERY_TIPO_ENTREGA{		
		REGULAR(1,"Regular"),
		EXPRESS(2, "Express"),
		SUPER_EXPRESS(3,"SuperExpress"),
		PROVINCIA(4,"Provincia"),
		REGULAR_PROGRAMADA(5,"Regular programada");
	...
	public  enum TIPO_PROVEEDOR_IBK{
		...
		PLAZAPOINTS(1,"PLAZAPOINTS","20548517151","PLAZAPOINTS - CUPONIUM"),
		COSTAMAR(2,"COSTAMAR","20794651352","COSTAMAR"),
		ZONACARDS(3,"ZONACARDS","20548517151","PLAZAPOINTS - CUPONIUM");
		...
		private TIPO_PROVEEDOR_IBK(int codigo, String descripcion, String ruc,String idProveedor){
			this.codigo=codigo;
			this.descripcion = descripcion;
			this.ruc = ruc;
	...
	public  enum ESTADO_RECLAMO{
		...
		/*public static String obtenerNombreEstado(Integer id){
			String strCodigoReturn=null;
			UtilEnum.ESTADO_RECLAMO[] valores = ESTADO_RECLAMO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo().equals(id)){
					strCodigoReturn=valores[i].getTexto();
					return strCodigoReturn;
		...
		public static String getEstadoReclamoTexto(Integer estado){
			String descripcion =  null;
			for(UtilEnum.ESTADO_RECLAMO item : UtilEnum.ESTADO_RECLAMO.values() ){
	...
	public enum TIPO_VUELO_COSTAMAR_ADMIN {
		VUELO_IDA_VUELTA(2, "IDA Y VUELTA"), 
		VUELO_IDA(1, "SOLO IDA");

		private final Integer codigo;
		private final String texto;

		private TIPO_VUELO_COSTAMAR_ADMIN(Integer codigo, String texto) {
			this.codigo = codigo;
			this.texto = texto;
		}
	...
	public  enum TIPO_POS{
		...
		public static String getTipoPos(Integer estado){
			String descripcion =  null;
			for(UtilEnum.TIPO_POS item : UtilEnum.TIPO_POS.values() ){
				if(item.getCodigo().equals(estado)){
					descripcion = item.getTexto();
					break;
				}
	...
	public  enum CANAL{
		...
		public static CANAL getCanal(Integer codigo){
			UtilEnum.CANAL[] valores = CANAL.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return valores[i];
...
//			UtilEnum.TIPO_OBSERVACION[] valores = TIPO_OBSERVACION.values();
//			for (int i = 0; i < valores.length; i++) {
...
//	}
	...
	public  enum ESTADO_CONSULTA{
		...
		public static ESTADO_CONSULTA obtenerConsulta(Integer codigoPrioridad){
			UtilEnum.ESTADO_CONSULTA[] valores = ESTADO_CONSULTA.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public  enum PRIORIDAD_CONSULTA_RECLAMO{
		...
		public static String obtenerTextoPrioridad(Integer codigoPrioridad){
			UtilEnum.PRIORIDAD_CONSULTA_RECLAMO[] valores = PRIORIDAD_CONSULTA_RECLAMO.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public  enum ESTADO_COBRO_LIQUIDACION{		
		PENDIENTE_CONFIRMACION(1,"Pendiente confirmacion", "Liquidado","01"),//ERA BORRADOR
		NO_PAGADO(8,"No pagado","Validado","02"), //0 ERA ACEPTADO
		PROCESO_PAGO(9,"Proceso pago","En proceso de pago","03"),//nuevo estado 
		PAGADO(11,"Pagado","Pagado","04"),//1 ERA
		...
		private ESTADO_COBRO_LIQUIDACION(int codigo, String texto, String filtro, String tracking){
			this.codigo = codigo;
			this.texto=texto;
			this.filtro = filtro;
		...
		public static ESTADO_COBRO_LIQUIDACION getEstadoCobroLiquidacion(Integer codigo){
			UtilEnum.ESTADO_COBRO_LIQUIDACION[] valores = ESTADO_COBRO_LIQUIDACION.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return valores[i];
				}
		...
		public static ESTADO_COBRO_LIQUIDACION getEstadoCobroLiquidacionTracking(String tracking){
			UtilEnum.ESTADO_COBRO_LIQUIDACION[] valores = ESTADO_COBRO_LIQUIDACION.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getTracking().equals(tracking)){
					return valores[i];
				}
			}
			return null;
	...
	public  enum TIPO_COMENTARIO_VENTA{		
		...
		public static TIPO_COMENTARIO_VENTA getTipoComentario(Integer codigoComentario){
			UtilEnum.TIPO_COMENTARIO_VENTA[] valores = TIPO_COMENTARIO_VENTA.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public  enum ESTADO_RESPUESTA{
		...
		public static String obtenerNombreEstado(Integer id){
			String strCodigoReturn=null;
			UtilEnum.ESTADO_RESPUESTA[] valores = ESTADO_RESPUESTA.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo().equals(id)){
					strCodigoReturn=valores[i].getTexto();
					break;
		...
		public static String getEstadoReclamoTexto(Integer estado){
			String descripcion =  null;
			for(UtilEnum.ESTADO_RESPUESTA item : UtilEnum.ESTADO_RESPUESTA.values() ){
				if(item.getCodigo().equals(estado)){
					descripcion = item.getTexto();
					break;
				}
	...
	public  enum TIPO_LIQUIDACION{
		PUNTOS_PLAZAPOINTS(1,"Puntos Plazapoints","PuntosPP","puntosPlazapoints",5),
		PUNTOS_COSTAMAR(2,"Puntos Costamar","PuntosCostamar","puntosCostamar",6),
		COMISIONES(3,"Comisiones","ComisionPP","comision",7), //solo se cobra por uso puntos de productos
		DEVOLUCIONES(4,"Devoluciones","DevolucionPP","devolucion",8),
		FLETE_VALE(5,"Flete de vale","FletePP","fletevale",9),
		CUPONES_PLAZAPOINTS(6,"Cupones plazapoints","CuponesPP","cupon",11),
		CUPONES_COSTAMAR(7,"Cupones costamar","CuponesCostamar","cupon",12),
		MARGEN_IBK(8,"Margen ibk","MargenIbk","margenIbk",14),
		DELIVERY_PRODUCTO(9,"Delivery Producto","Delivery","deliveryProducto",15),
		MACRO_LIQUIDACION(20,"Macro liquidacion","Liquidacion","macro-liquidacion",20);
		...
		public static TIPO_LIQUIDACION getTipoLiquidacion(Integer codigoPrioridad){
			UtilEnum.TIPO_LIQUIDACION[] valores = TIPO_LIQUIDACION.values();
			for (int i = 0; i < valores.length; i++) {
				if(valores[i].getCodigo() == codigoPrioridad){
					return valores[i];
				}
	...
	public static enum CATEGORIA_ENTRETENIMIENTO{
		...
		public static CATEGORIA_ENTRETENIMIENTO getCategoriaXCodigo(String codigoCategoria){
			UtilEnum.CATEGORIA_ENTRETENIMIENTO[] valores = CATEGORIA_ENTRETENIMIENTO.values();
			for (int i = 0; i < valores.length; i++) {
...
//        <option value="3">Internacional</option>
	...
	public static enum TIPO_RESTRICCION_JOINNUS{
		
		VALIDO_PUBLICO_GENERAL("1","Válido para público en general"),
		MENOR_EDAD_ACOMPANHADO("2","Menores de edad acompañados por un adulto"),
		MAYOR_EDAD("3","Evento para mayores de edad");
		
		private final String codigo;
		private final String nombre;
		
		public static TIPO_RESTRICCION_JOINNUS obtenerTipoAnchoByDescripcion(String codigo){
			TIPO_RESTRICCION_JOINNUS[] valores = TIPO_RESTRICCION_JOINNUS.values();
			TIPO_RESTRICCION_JOINNUS tipoAnchos=null;
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo().equalsIgnoreCase(codigo)){
		...
		private TIPO_RESTRICCION_JOINNUS(String codigo, String nombre) {
			this.codigo = codigo;
			this.nombre = nombre;
		}
	...
	public static enum PROCESO_COLA{
		...
		TRANSFERIR_VUELOS_ASINCRONO("TRANSFERIR-VUELOS-ASINCRONO"), 
		NOTIFICAR_RECLAMO_NO_CERRADO("NOTIFICACION RECLANO NO CERRADO"),
		NOTIFICAR_RECLAMO_NO_RESPONDIDO("NOTIFICACION RECLANO NO RESPONDIDO")
		;
	...
	public static enum REPORTE_COLA{
		...
		REPORTE_VENTA_ENTREGA(13, "COLA-REPORTES-PROCESAR-EXCEL", "REPORTES-VENTA-ENTREGA", "Reporte-venta-entrega-"),
		MARGEN_IBK(14,"COLA-REPORTES-PROCESAR-EXCEL","LIQUIDACION-MARGEN-IBK","Liquidacion-margen-ibk-"),
		DELIVERY_PRODUCTO(15,"COLA-REPORTES-PROCESAR-EXCEL","LIQUIDACION-DELIVERY-PRODUCTO","Liquidacion-delivery-producto"),
		MACRO_LIQUIDACION(20,"COLA-REPORTES-PROCESAR-EXCEL","LIQUIDACIONES MACRO","LiquidacionMacro-");
		...
		public static REPORTE_COLA getReporteXId(int idReporte){
			UtilEnum.REPORTE_COLA[] valores = REPORTE_COLA.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public static enum TIPO_CONCILIACION{
		...
		public static TIPO_CONCILIACION getTipoConciliacion(String codigoPrioridad){
			UtilEnum.TIPO_CONCILIACION[] valores = TIPO_CONCILIACION.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public static enum TIPO_TX_REFERENCIA{
		...
		public static TIPO_TX_REFERENCIA getTipoTxReferencia(String codigoPrioridad){
			UtilEnum.TIPO_TX_REFERENCIA[] valores = TIPO_TX_REFERENCIA.values();
			for (int i = 0; i < valores.length; i++) {
				if(valores[i].getTexto().equals(codigoPrioridad)){
					return valores[i];
				}
	...
	public static enum TIPO_MOTIVO_LIQUIDACION{
		...
		public static TIPO_MOTIVO_LIQUIDACION getMotivoPorCodigo(Integer codigo){
			UtilEnum.TIPO_MOTIVO_LIQUIDACION[] valores = TIPO_MOTIVO_LIQUIDACION.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo() == codigo){
					return  valores[i];
				}
	...
	public static enum TIPO_EVENTO{
		...
		BANNER_JOINNUS(26,"BANNER_JOINNUS"),
		BANNER_INFERIOR(27,"BANNER_INFERIOR");
		...
		public static TIPO_EVENTO getIdEvento(Integer codigo){
			UtilEnum.TIPO_EVENTO[] valores = TIPO_EVENTO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum CODIGO_TIPO_CATEGORIA{
		...
		public static CODIGO_TIPO_CATEGORIA getCodigoTipoCategoria(String alias){
			UtilEnum.CODIGO_TIPO_CATEGORIA[] valores = CODIGO_TIPO_CATEGORIA.values();
			for(int i=0; i<valores.length; i++){
	...
	public static enum REPORTE_LIQUIDACION_COLA{
		...
		public static REPORTE_LIQUIDACION_COLA getReporteXId(int idReporteLiquidacion){
			UtilEnum.REPORTE_LIQUIDACION_COLA[] valores = REPORTE_LIQUIDACION_COLA.values();
			for (int i = 0; i < valores.length; i++) {
	...
	public static enum TIPO_CUPON_DESCUENTO{
		...
		public static TIPO_CUPON_DESCUENTO getTipoCuponDescuento(Integer codigo){
			UtilEnum.TIPO_CUPON_DESCUENTO[] valores = TIPO_CUPON_DESCUENTO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum GRUPO_CUPON_DESCUENTO{
		...
		public static GRUPO_CUPON_DESCUENTO getGrupoCupon(Integer codigo){
			UtilEnum.GRUPO_CUPON_DESCUENTO[] valores = GRUPO_CUPON_DESCUENTO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum EVENTO_REDIRECCION{
		NO(0,"No",false,"#"),
		LOGIN(1,"Login",true,"%s/ingresar"),
		REGISTRATE(2,"Registrarte",true,"%s/registrate"),
		CUPON(3,"Cupon",true,"%s/cliente/perfil"),
		COMPRAS(4,"Categorias",true,"%s/categorias"),
		PRODUCTO(20,"Producto",true,"%s/p/"),
		CATEGORIA(21,"Categoria",true,"%s/c/"),
		OTRO(50,"Otros",true,"");
		...
		public static EVENTO_REDIRECCION getIdEvento(Integer codigo){
			UtilEnum.EVENTO_REDIRECCION[] valores = EVENTO_REDIRECCION.values();
			for(int i=0; i<valores.length; i++){
	...
	public static enum TIPO_EVALES{
		...
		public static TIPO_EVALES getTipoEvales(Integer codigo){
			UtilEnum.TIPO_EVALES[] valores = TIPO_EVALES.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum TOKEN_TIPO_PROCESO{
		...
		public static TOKEN_TIPO_PROCESO getTokenTipoProceso(Integer codigo){
			UtilEnum.TOKEN_TIPO_PROCESO[] valores = TOKEN_TIPO_PROCESO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum TIPO_DIRECTORIO{
		...
		public static TIPO_DIRECTORIO getTipoDirectorio(Integer codigo){
			UtilEnum.TIPO_DIRECTORIO[] valores = TIPO_DIRECTORIO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum TIPO_TOP_PRODUCTOS {
		...
		public static TIPO_TOP_PRODUCTOS getCodigoTopProductos(String codigo){
			UtilEnum.TIPO_TOP_PRODUCTOS[] valores = TIPO_TOP_PRODUCTOS.values();
			for(int i = 0; i < valores.length; i++){
				if(valores[i].getCodigo().equals(codigo)){
					return  valores[i];
				}
	...
	public static enum TIPO_VUELO {
		...
		TODAS("A","Todas"),
		ECONOMY("M","Economic"),
		ECONOMY_PREMIUN("W","Economic Premium"),
		FIRST("F","First"),
		BUSINESS("C","Business");
		...
		public static TIPO_VUELO getTipoClaseVuelo(String codigo){
			UtilEnum.TIPO_VUELO[] valores = TIPO_VUELO.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo().equals(codigo)){
					return  valores[i];
				}
	...
	public static enum EXCEL_BASE_CAMPOS {
		...
		ES_OFERTA(40,"es_oferta",EXCEL_BASE_VALIDACION.NO_VACIO),
		ES_SUBVENCION(41,"es_subvencion",EXCEL_BASE_VALIDACION.NO_VACIO),
		SUBVENCION_IBK(42,"subvencion_ibk",EXCEL_BASE_VALIDACION.DOUBLE),
		GRUPO_CARACTERISTICAS(43,"grupo_caracteristicas",EXCEL_BASE_VALIDACION.CADENA),

		ES_RESTRICCION_COMPRA(44,"es_restriccion_compra",EXCEL_BASE_VALIDACION.NO_VACIO),
		MAXIMA_CANTIDAD(45,"maxima_cantidad",EXCEL_BASE_VALIDACION.DIGITO),
		PERIODO_DIAS(46,"periodo_dias",EXCEL_BASE_VALIDACION.DIGITO),
		MARGEN_BGM(47,"margen_bgm",EXCEL_BASE_VALIDACION.DOUBLE),
		MARGEN_IBK(48,"margen_ibk",EXCEL_BASE_VALIDACION.DOUBLE),
		FLETE_IBK(49,"flete_ibk",EXCEL_BASE_VALIDACION.DOUBLE)
		...
		public static EXCEL_BASE_CAMPOS getExcelBaseCampo(String nombre){
			UtilEnum.EXCEL_BASE_CAMPOS[] valores = EXCEL_BASE_CAMPOS.values();
			for(int i=0; i<valores.length; i++){
	...
	public static enum EXCEL_BASE_VALIDACION {
		...
		PAR_TIPO_PESO(7,"debe tomar valores de "+UtilEnum.TIPO_PESO.VOLUMETRICO.getTexto()+" o "+UtilEnum.TIPO_PESO.LINEAL.getTexto(),null),
		PAR_TIPO_VENTA(8,"debe tomar valores de "+UtilEnum.TIPO_VENTA_PRODUCTO.SUBVENCIONADO.getTexto()+" o "+UtilEnum.TIPO_VENTA_PRODUCTO.NO_SUBVENCIONADO.getTexto(),null);
		private final Integer codigo;
		private final String  texto;			
	...
	public static enum TIPO_DATOS_CARGA_FILE
	{
		...
		public static TIPO_DATOS_CARGA_FILE getTipoDatosCargaFile(Integer codigo){
			UtilEnum.TIPO_DATOS_CARGA_FILE[] valores = TIPO_DATOS_CARGA_FILE.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum EVENTO_TIPO_FIGURA
	{
		CUADRADO(1,"Cuadrado"),
		RECTANGULO(2,"Rectangulo");
		
		private final Integer codigo;
		private final String texto;
		
		private EVENTO_TIPO_FIGURA(Integer codigo, String texto) {
			this.codigo = codigo;
			this.texto = texto;
		}
		
		public static EVENTO_TIPO_FIGURA getEventoTipoFigura(Integer codigo){
			UtilEnum.EVENTO_TIPO_FIGURA[] valores = EVENTO_TIPO_FIGURA.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum EVENTO_TIPO_ACCION_LOGIN
	{
		NO_LOGIN(1,"No login"),
		LOGIN(2,"Login"),
		AMBOS(3,"Ambos");
		...
		private EVENTO_TIPO_ACCION_LOGIN(Integer codigo, String texto) {
			this.codigo = codigo;
			this.texto = texto;
		}
		
		public static EVENTO_TIPO_ACCION_LOGIN getEventoTipoAccion(Integer codigo){
			UtilEnum.EVENTO_TIPO_ACCION_LOGIN[] valores = EVENTO_TIPO_ACCION_LOGIN.values();
			for(int i=0; i<valores.length; i++){
				if(valores[i].getCodigo()==codigo){
					return  valores[i];
				}
	...
	public static enum VARIACION_PRECIO_PRODUCTO{
		PRECIO_VARIABLE(1,"Precio variable"),
		PRECIO_SUBVENCIONADO(2,"Precio subvencionado");
		private final int codigo;
		private final String texto;
		
		private VARIACION_PRECIO_PRODUCTO(int codigo, String texto) {
			this.codigo = codigo;
			this.texto = texto;
		}
		public int getCodigo() {
